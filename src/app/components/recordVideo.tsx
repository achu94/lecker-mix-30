...
import { useLoadingRing } from "@/hooks/useLoadingRing";
...
export default function RecordVideo() {
    ...
    const { withLoading, loadingUi, isLoading } = useLoadingRing();
    ...
    const finishRecording = async () => {
        await withLoading(async () => {
            ...
            try {
                for (const blobUrl of recoredVideoList) {
                    // Hole Blob aus blob-URL
                    const response = await fetch(blobUrl);
                    const blob = await response.blob();

                    setIsUploading(true);

                    // Erstelle FormData und hänge das Video an
                    const formData = new FormData();
                    formData.append("video", blob, "recorded-video.webm");

                    // Sende das Video an deinen Backend-API-Endpunkt
                    ...
                    // Optional: Antwort vom Server lesen
        ...
        <div className="fixed inset-0 bg-black z-50 flex items-center justify-center">
            <div className="relative w-[90vw] max-w-[420px] aspect-9/16 overflow-hidden rounded-2xl shadow-2xl border border-gray-800 bg-black">
                ...
                {isLoading
                    ...
                    ? <div className="text-white h-full flex justify-center flex-col items-center">
                        {loadingUi}
                        <p className="font-bold text-center">Ihr Video wird vorbereitet und hochgeladen. Bitte schließen Sie das Fenster nicht</p>
                    </div>
                    : (
                        <>
                            {/* 🎥 Main video area */}
                            {isLive ? (
                                <VideoPreview stream={cameraStream} error={error} />
                            ) : (
                                <video
                                    src={previewVideoUrl}
                    ...
                    ${isDragging
                            ...
                            >
                                {recoredVideoList.map((video, idx) => (
                                    <div
                                        key={idx}
                                        className={`relative group shrink-0 transition-all duration-200 rounded-md
                            ${isDragging
